...
// --- Lightweight UI primitives (so the file runs standalone in Canvas)
...
export default function PortfolioPage() {
  ...
  const usdApprox = (kgs: number) => (effRate ? formatUSD(kgs / effRate) : "");
      ...
      <Section className="pt-16">
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <p className="mt-4 text-white/60">{t.minTicket} <span className="text-white/70">(≈ {effRate ? `${formatUSD(500000 / effRate)}` : "…"})</span></p>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 text-white/70 text-sm mb-6">
          {!manualMode ? (
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-white/80">{lang==='ru' ? 'Курс (вручную), USD/KGS:'}</span>
              <input
                type="number"
                inputMode="decimal"
                step="0.0001"
                    ...
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="rounded-lg border border-white/20 bg-[#0A0F1C]/95 p-3 text-sm text-white shadow-xl">
                          ...
                          <div className="opacity-90">{lang==='ru' ? 'Выплата' : 'Payout'}: <span className="font-semibold">{formatKGS(p.payout)}{` (≈ ${effRate ? formatUSD(p.payout / effRate) : '…'})`}</span></div>
                          <div className="opacity-90">{lang==='ru' ? 'Итоговая сумма' : 'Final amount'}: <span className="font-semibold">{formatKGS(p.total)}{` (≈ ${effRate ? formatUSD(p.total / effRate) : '…'})`}</span></div>
                ...
                <tbody>
                  {Array.from({ length: months }).map((_, i) => (
                    <tr key={i} className="border-t border-white/15 hover:bg-white/10">
                      ...
                      <td className="px-3 py-2 font-medium">{formatKGS(amount)} <span className="text-white/70">(≈ {effRate ? usdApprox(amount) : '…'})</span></td>
                      <td className="px-3 py-2 font-semibold text-white">{formatKGS(amount * (pickTier(months).net / 12))} <span className="text-white/70">(≈ {effRate ? usdApprox(amount * (pickTier(months).net / 12)) : '…'})</span></td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t border-white/30 font-semibold text-white">
                    ...
                    <td className="px-3 py-2">{formatKGS((amount * (pickTier(months).net / 12)) * months)} <span className="text-white/70">(≈ {effRate ? usdApprox((amount * (pickTier(months).net / 12)) * months) : '…'})</span></td>
                  </tr>
                </tfoot>
              </table>
            </div>
